...
import { SimReport } from "./types/simReport";
import {
  S3Client,
  PutObjectCommand
...
export const handler = async (payload: string) => {
  ...
  reports.forEach((report) => {
    if (!report.error) {
      reportIdsUrls.push(RAIDBOTS_REPORT_URL + report.simId + RAIDBOTS_SUFFIX);
    }
  });
  ...
  const sorted = reportsData
    .map((report) => {
      ...
      const ilvl =
        report.simbot.meta.rawFormData.character.items.averageItemLevelEquipped;
  ...
  sorted.forEach((char, i) => {
    const formatted = char.mean
      .toString()
      .replace(/\B(?=(\d{3})+(?!\d))/g, " ");
    message += `${i + 1}.\t**${char.charName}** (${char.ilvl})\n\t\`${formatted} dps\n\``;
  });
